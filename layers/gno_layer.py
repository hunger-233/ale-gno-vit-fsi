...
from neuralop.layers.channel_mlp import LinearChannelMLP as MLPLinear
...
class GnoPremEq(nn.Module):
    def __init__(
        ...
        print(self.var_encoding_channels)

        # project to higher dim[13,10,60]
        ...
        for i in range(n_layers):
            self.it.append(IntegralTransform(
                channel_mlp_layers=self.mlp_layers,
    ...
    def update_grid(
        ...
        for key, value in self.neighbour.items():
            self.neighbour[key] = value.to(input_grid.device)
        ...
        for key, value in self.neighbour_last.items():
            self.neighbour_last[key] = value.to(input_grid.device)
...
class GNO(nn.Module):
    def __init__(self, in_dim, out_dim,
        ...
        # Move the neighbour data to GPU (if necessary)
        for key, value in self.neighbour.items():
            self.neighbour[key] = value.to(input_grid.device)

        self.it = IntegralTransform(channel_mlp_layers=self.mlp_layers)
    ...
    def forward(self, inp):
        ...
        # inp = inp.reshape((inp.shape[0],inp.shape))  # 去除第二维，变成 (batch_size, n_points, in_dim)
        batch_size = inp.shape[0]  # Get batch size from the input
        try:
            x = inp
            x = rearrange(
                x,
            ...
            pass
        # print('gno x {}'.format(x.shape))  # Check output shape

        # Project input through the projection layer (supports batch size)
        x = self.projection(x)  # Shape: (batch_size, n_points, out_dim)
        # print('gno pro x {}'.format(x.shape))  # Check output shape

        out = self.it(self.input_grid, self.neighbour,
                      self.output_grid, x)  # Pass through Integral Transform


        # print('gno x {}'.format(out.shape))  # Check output shape

        # Ensure the output is returned with the batch dimension
        return out  # output shape: (batch_size, n_points, out_dim)

...
class VitGno(nn.Module):
    ...
    def forward(
        ...
        if self.lifting:
            # print("In Lifting")
            x = self.lifting(inp)#[1,2,10000,128]
            # x = x[0]#[2,10000,128]
            # print('模型经过lifting {}'.format(x.shape))
            x = rearrange(x, 'b (h w) c -> b c h w', h=self.grid_size[0])#[2,128,250,40]
        else:
            x = inp
        
        # print('模型经过lifting 1 {}'.format(x.shape))

        # for layer_idx in range(self.n_layers):
        #     x = self.base_GNN[layer_idx](x)

        for _ in range(self.n_layers_transformer):
            x = self.base(x)
            # print('模型经过VIT {}'.format(x.shape))#[1,500,20]
            x = self.expander(x)
            # print('模型经过expander {}'.format(x.shape))#[2,500,20]
            x = x.reshape(-1, self.hidden_dim,
                        self.grid_size[0], self.grid_size[1])
            # print('模型经过reshape {}'.format(x.shape))#[2,500,20]
        ...
        if self.projection:
            ...
            # print('模型经过rearrange {}'.format(x.shape))
            x = self.projection(x)
        # print('模型经过projection {}'.format(x.shape))
        return x
